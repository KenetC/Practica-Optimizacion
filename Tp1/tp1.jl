...
begin
	using LinearAlgebra, PlutoUI, Plots, DelimitedFiles,Images,ImageView
	TableOfContents()
end

# ╔═╡ 29acc529-12be-401f-beb3-8bf83a5651ba
include("funciones_aux.jl")
...
# ╔═╡ 42f9c757-9ecd-423e-a12a-71b9df91ef8e
begin 
	D = Datos()
	grafico(D)
end
...
# ╔═╡ 20d4b2ac-28b2-4cdc-ad40-1d222796c795
# D son los datos dados de forma [x,y]
function def_problem(D)
	A = hcat(2*D[:,1],2*D[:,2],ones(size(D,1)))
	p = D[:,1] .^2 + D[:,2] .^2
	return A,p
end
...
# ╔═╡ 7879c54d-78a0-4916-9c48-0fcbfcc3620f
md"""
como queremos resolver el problema 
 ```math
 min_{x} ||Ax-p||_2^2
```
si concideramos la descomposicion QR de $A$, vamos a suponer que $rg(A)=3$, luego tenemos 
...
```math  
 min_{x} ||Q^tAx  - Q^t p||^2_2 = min_{x}||R_1 x -p^{(n)}||^2_2 + ||p^{(m-n)}||^2_2
```
donde  $Q^t p = (p^{(n)},p^{(m-n)})$, por lo que nuestra solucion sera 

$x = R_1^{-1}p^{(n)}$

"""

# ╔═╡ f64661f8-e5a4-476e-8ac4-098cccd41513
function ajuste_alg(D)
	A,p = def_problem(D)
	Q,R = qr(A)
	#Q = Q[:,1:3]
	pₙ = (Q' * p)[1:3]
...
begin 
	graficos = []
	for i in 1:10
		push!(graficos,plot_algebraico(D,i))
	end	
end

# ╔═╡ 041dfcbd-c23f-4a98-b490-3c3ea4df1775
load(graficos[5])
...
# ╔═╡ 22254d09-3683-4522-8234-c195b9a62a65
begin 
	function d(D::Matrix{Float64},p)::Vector{Float64}
		a,b,r = p
		X = D[:,1]
		Y = D[:,2]
		R = ((X.-a).^2+(Y.-b).^2).^(1/2) .- r
	...
	F(D::Matrix{Float64},p::Vector{Float64}) = sum(d(D,p).^2)
	
	function Jacob(D::Matrix{Float64},p)::Matrix{Float64}
		X = D[:,1]
		Y = D[:,2]
		a,b,r = p
		n = length(X)
...
begin 
	function paso(J::Matrix{Float64},d::Vector{Float64})
		U,Σ,V = svd(J)
	...
	function ajuste_geom(D::Matrix{Float64},N,TOL)
		a,b,c = ajuste_alg(D)
		r =  √(c+a^2+b^2)
		p = a,b,r
		k = 1 
		while k < N 
			J = Jacob(D,p)
			h = paso(J,d(D,p))
			if (h == "Numero de condicion muy alto") 
				return "Numero de condicion muy alto"
			end
			p = p .- h 
			if abs(F(D,p)) < TOL
...
# ╔═╡ bbf928a4-c504-4d99-a87f-7b2ab898e2cb
begin 
	graficos2 = [plot_conjunto(D, i) for i in 1:10 if plot_conjunto(D, i) != "No se pudo"]
end
...
manifest_format = "2.0"
project_hash = "2edf0866ccbaec8b3cc35fa0dfc4d5047a40da37"
...
git-tree-sha1 = "23cff0d65090568963632447dd295bfae729b2ed"
