...
"""
cross_in_tray(x,y) = -1e-4*((abs(sin(x)*sin(y)exp(abs(100-sqrt(x^2+y^2)/pi)))+1)^0.1)
cross_in_tray(x::Vector{Float64}) = cross_in_tray(x[1],x[2])
...
"""
eggholder(x,y) = -(x+47)sin(sqrt(abs(y + x/2 + 47)))-x*sin(sqrt(abs(x-(y+47))))
eggholder(x::Vector{Float64}) = eggholder(x[1],x[2])
...
holder_table(x::Vector{Float64})   = holder_table(x[1],x[2]) 
...
bochavesky(x::Vector{Float64}) = bochavesky(x[1],x[2])
...
mccormick(x::Vector{Float64}) = mccormick(x[1],x[2])
"""
    power_sum function
"""  
...
goldstein_price(x) = goldstein_price(x[1],x[2])
"""
    perm_d_beta
    minimiser : x = (1,2,...,d)
"""
