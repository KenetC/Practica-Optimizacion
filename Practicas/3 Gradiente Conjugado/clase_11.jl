...
using LinearAlgebra, Plots, PlutoUI ,Symbolics
...
function FR_method(f::Function,n::Integer;x0=0,ϵ=0.001,N=100)
	x = [Symbol("x$i") for i in 1:n]
	@variables x[1:n]
	g(x...) = f([x...]) # va depender como tengamos definida la funcion 
	grad = Symbolics.gradient(g(x...), [x...])
	if x0 == 0 
		x0 = zeros(n)
	end
...
# ╔═╡ 5ee301a0-2f08-4550-9774-d447cfee8e67
begin 
	f(x) = 100*(x[2]-x[1]^2)^2 + (1-x[1])^2 
	ϵ = 1e-6; x01 = [-2,2]; x02 = [2,-2]; x03 = [-2,-2]
end 

# ╔═╡ aa9db9bc-0124-4016-afde-23aa79e92b64
begin 
	xsol1 = FR_method(f,2,x0 = x01,ϵ=ϵ)
	xsol2 = FR_method(f,2,x0 = x02,ϵ=ϵ)
	xsol3 = FR_method(f,2,x0 = x03,ϵ=ϵ)
	println("solucion 1: ",xsol1)
	println("solucion 2: ",xsol2)
	println("solucion 3: ",xsol3)
end 

# ╔═╡ 108bd597-056e-4f16-9c5a-71d2e5eb83b3
begin 
	ϕ(x) = x^2-x
	phi0 = ϕ(0)
end

# ╔═╡ 3c8526cb-372c-4875-a90d-53fea489db35
begin
o = 0.35;; η = 1.3
while ϕ(o) <= phi0
		o *= η  
...
function generate_initial_points(n, r, L)
    points = []
...
initial_points = generate_initial_points(n, r, L)

# Plotear los círculos y el cuadrado
plot(xlim=(0, 1), ylim=(0, 1), aspect_ratio=:equal, legend=false)
for point in initial_points
    x, y = point
    circle = Shape(x .+ r*cos.(0:0.01:2π), y .+ r*sin.(0:0.01:2π))
    plot!(circle, fillalpha=0.3, linecolor=:blue)
end
plot!([0, 1, 1, 0, 0], [0, 0, 1, 1, 0], linewidth=2, linecolor=:black)  # Cuadra
end
...
manifest_format = "2.0"
project_hash = "5f5c5cd989ae8b2ea0564d9b9c7a4280c40d784a"
